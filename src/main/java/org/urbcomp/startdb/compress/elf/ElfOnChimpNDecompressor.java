...
public class ElfOnChimpNDecompressor {
    ...
    private ChimpNDecompressor chimpNDecompressor;


    public ElfOnChimpNDecompressor(byte[] bs,int previousValues) {
        in = new InputBitStream(bs);
        chimpNDecompressor = new ChimpNDecompressor(in,previousValues);
    }
    ...
    public Double readValue() {
        ...
        if (chimpNDecompressor.getEndOfStream()) {
            return null;
        }
        if(flag==1){
    ...
    private void next() throws IOException {
        ...
        Long store = chimpNDecompressor.readLongValue();
        if (!chimpNDecompressor.getEndOfStream()) {
            storedVal=store;
        }
    }
